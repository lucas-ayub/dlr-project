...
def getRawData1D(ptgs, ptx, prx, vtx, vrx, ta, sq_tx, sq_rx,
    ...
    for p_idx in range(Np):
        # Reset per point: each scatterer has its own antenna illumination window.
        wa_tx = np.zeros(Na)
        wa_rx = np.zeros(Na)
        ...
        rh = rh_ms + rh_bs
        # Accumulate contributions: each scatterer adds coherently to the
        # received signal instead of overwriting the previous one.
        datal += aPattern * np.exp(-2j * np.pi * rh / wl)
    return datal


def generate_reference(cfg: ExperimentConfig, tracks: PlatformTracks) -> np.ndarray:
    """Monostatic reference signal, summed over every scatterer in the scene."""
    ptgs = cfg.scene.points
    return getRawData1D(
        ptgs, tracks.ptx, tracks.ptx, tracks.vtx, tracks.vtx, cfg.ta,
...
def generate_channels(cfg: ExperimentConfig, tracks: PlatformTracks) -> np.ndarray:
    """Per-receiver bistatic channels, summed over every scatterer, subsampled by Nrx."""
    Nrx = cfg.Nrx
    ptgs = cfg.scene.points
    s_channel = np.zeros([Nrx, cfg.Na_ch], dtype=np.complex128)
    for ii in range(Nrx):
        s_channel[ii, :] = getRawData1D(
            ptgs, tracks.ptx, tracks.prx[ii, :, :],
